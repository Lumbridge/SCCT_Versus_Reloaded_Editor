...
{
    readonly TextBox source = new(), scct = new(), output = new(), package = new();
    readonly ComboBox map = new() { DropDownStyle = ComboBoxStyle.DropDownList };
    readonly Button scan = new() { Text = "Find paths" }, inspect = new() { Text = "Inspect map" }, convert = new() { Text = "Convert to SCCT" }, cancel = new() { Text = "Cancel", Enabled = false };
    ...
    public MainForm()
    {
        ...
        layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 80)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 160)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 52)); layout.RowStyles.Add(new RowStyle(SizeType.Percent, 100)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 42));
        layout.Controls.Add(new Label { Text = "SCDA → SCCT", Font = new Font("Segoe UI", 25, FontStyle.Bold), AutoSize = true }, 0, 0);
        var fields = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 5 }; fields.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 180)); fields.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));
        AddField(fields, 0, "Double Agent MapsPC", source); AddField(fields, 1, "Source map", map); AddField(fields, 2, "SCCT editor folder", scct); AddField(fields, 3, "Output folder", output); AddField(fields, 4, "Package name", package); layout.Controls.Add(fields, 0, 1);
        var buttons = new FlowLayoutPanel { Dock = DockStyle.Fill }; foreach (var b in new[] { scan, inspect, convert, cancel }) { b.AutoSize = true; b.Height = 36; buttons.Controls.Add(b); } layout.Controls.Add(buttons, 0, 2);
        layout.Controls.Add(log, 0, 3); var footer = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 2 }; footer.Controls.Add(status, 0, 0); footer.Controls.Add(progress, 0, 1); layout.Controls.Add(footer, 0, 4); Controls.Add(layout);
        source.Text = @"C:\Users\ryans\Desktop\Enhanced SCDA Online 2.2\Packages\_Common\MapsPC"; output.Text = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "SCDAExports"); package.Text = "SCDA_Map";
        scan.Click += (_, _) => Scan(); inspect.Click += (_, _) => Inspect(); convert.Click += async (_, _) => await ConvertAsync(); cancel.Click += (_, _) => cancellation?.Cancel(); Shown += (_, _) => Scan();
    }
    ...
    async Task ConvertAsync()
    {
        ...
        cancellation = new CancellationTokenSource(); SetBusy(true); progress.Value = 5;
        try
        {
            await Task.Run(() =>
            {
                var p = Probe(maps[map.SelectedIndex]); cancellation.Token.ThrowIfCancellationRequested();
                var folder = Path.Combine(output.Text, Path.GetFileNameWithoutExtension(p.File) + "_SCCT"); Directory.CreateDirectory(folder);
                ...
                File.WriteAllText(Path.Combine(folder, "conversion-status.md"), $"# {Path.GetFileNameWithoutExtension(p.File)}\n\nSCDA container: `{p.Magic}`\n\nStatus: {p.Status}\n\nTarget package: `{package.Text}`\n\nCompanion files:\n" + string.Join("\n", p.CompanionFiles.Select(f => "- " + f)));
            }, cancellation.Token);
            progress.Value = 100; Log("Conversion workspace prepared. SCDA geometry decoding is the next backend stage."); status.Text = "Conversion stopped at container decode; diagnostics are ready.";
        }
        catch (OperationCanceledException) { status.Text = "Cancelled."; Log("Conversion cancelled; partial diagnostics were retained."); }
        finally { SetBusy(false); cancellation.Dispose(); cancellation = null; }
    }
    void SetBusy(bool value) { source.Enabled = map.Enabled = scct.Enabled = output.Enabled = package.Enabled = scan.Enabled = inspect.Enabled = convert.Enabled = !value; cancel.Enabled = value; }
